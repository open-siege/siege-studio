...
json joystick_to_json(SDL_Joystick* joystick, int device_index)
{
  ...
  result["vendorId"] = to_hex(Siege_JoystickGetVendor(joystick));
  result["productId"] = to_hex(Siege_JoystickGetProduct(joystick));
  ...
  if (SDL_IsGameController(device_index))
  {
    ...
    for (auto i = 0; i < int(SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_MAX); ++i)
    {
      ...
      result["bindings"]["buttons"][Siege_GameControllerGetStringForButton(controller.get(), button_type)] = to_json(binding);
    }
  }
