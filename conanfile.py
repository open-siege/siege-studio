...
class SiegeLauncherConanFile(ConanFile):
    ...
    def requirements(self):
        if self.settings.os == "Windows":
            ...
            self.run(f"conan install detours-conanfile.py -s build_type=Release -s compiler.runtime=static -s arch={self.settings.arch} --build=missing -of siege-modules/extension/siege-extension/detours")
