...
namespace win32::com
{
  class StdStreamRefBase : public ComObject
    , public ::IStream
  {
  public:
  ...
  public:
    StdIStreamRef(std::istream& stream, std::optional<std::size_t> size = std::nullopt) : stream(stream), size(size)
    {
    }
  ...
  template<typename TIStream = std::unique_ptr<std::istream>>
  class OwningStdIStream : public StdIStreamRef
  {
    TIStream stream;

  public:
    OwningStdIStream(TIStream stream, std::optional<std::size_t> size = std::nullopt) : 
        StdIStreamRef(*stream, size), stream(std::move(stream))
    {
    }
  };
  ...
  class StdIOStreamRef : public StdIStreamRef
    , public StdOStreamRef
  {
  public:
