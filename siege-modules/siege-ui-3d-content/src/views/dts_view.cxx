    ...
    if (!materials.empty())
    {
      ...
      for (auto i = 0u; i < materials.size(); ++i)
      {
        ...
        if (!material.filename.empty())
        {
          ...
          {
            const auto ext = platform::to_lower(std::filesystem::path(material.filename).extension().string());
            ...
            const auto predicate = [&](const auto& material_info){
              return material_info.filename == material.filename || platform::to_lower(material_info.filename.string()) == platform::to_lower(material.filename);
            };
