...
HRESULT get_window_class_for_stream(storage_info* data, wchar_t** class_name) noexcept
{
  ...
  auto window = this_module.CreateWindowExW(CREATESTRUCTW{
    ...
    .lpszClass = MAKEINTATOM(cfg_view_atom) });
  ...
  LPVOID lpvReserved) noexcept
{
  ...
  if (fdwReason == DLL_PROCESS_ATTACH || fdwReason == DLL_PROCESS_DETACH)
  {
    ...
    {
      cfg_view_atom = register_cfg_view(win32::window_module_ref(hinstDLL));
    }
    else if (fdwReason == DLL_PROCESS_DETACH)
    {
      ::UnregisterClassW(MAKEINTATOM(cfg_view_atom), hinstDLL);
    }
  }
