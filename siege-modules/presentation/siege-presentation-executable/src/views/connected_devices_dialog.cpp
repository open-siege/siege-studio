...
namespace siege::views
{
  ...
  struct connected_devices_dialog : win32::basic_window<connected_devices_dialog>
  {
    ...
    struct
    {
      ...
      win32::track_bar left_x_axis;
      win32::track_bar left_y_axis;
      win32::track_bar right_x_axis;
      win32::track_bar right_y_axis;
    } controller_controls;
    ...
    LRESULT wm_create()
    {
      ...
      controller_controls.left_trigger = *win32::CreateWindowExW<win32::track_bar>({ .hwndParent = *this,
        .style = WS_CHILD | WS_DISABLED | TBS_VERT | TBS_NOTICKS | TBS_TRANSPARENTBKGND,
        .lpszName = L"LT" });
      ...
      controller_controls.left_x_axis = *win32::CreateWindowExW<win32::track_bar>({ .hwndParent = *this,
        .style = WS_CHILD | WS_DISABLED | TBS_HORZ | TBS_NOTICKS | TBS_TRANSPARENTBKGND,
        .lpszName = L"LS X-Axis" });

      controller_controls.left_y_axis = *win32::CreateWindowExW<win32::track_bar>({ .hwndParent = *this,
        .style = WS_CHILD | WS_DISABLED | TBS_VERT | TBS_NOTICKS | TBS_TRANSPARENTBKGND,
        .lpszName = L"LS Y-Axis" });

      controller_controls.right_x_axis = *win32::CreateWindowExW<win32::track_bar>({ .hwndParent = *this,
        .style = WS_CHILD | WS_DISABLED | TBS_HORZ | TBS_NOTICKS | TBS_TRANSPARENTBKGND,
        .lpszName = L"RS X-Axis" });

      controller_controls.right_y_axis = *win32::CreateWindowExW<win32::track_bar>({ .hwndParent = *this,
        .style = WS_CHILD | WS_DISABLED | TBS_VERT | TBS_NOTICKS | TBS_TRANSPARENTBKGND,
        .lpszName = L"RS Y-Axis" });
    ...
    std::optional<win32::lresult_t> wm_size(std::size_t type, SIZE client_size)
    {
      ...
      auto slider_x_size = SIZE{ .cx = button_size.cx * 3, .cy = button_size.cy };
      auto slider_y_size = SIZE{ .cx = button_size.cx, .cy = button_size.cy * 3 };
      ...
      controller_controls.dpad_left_button.SetWindowPos(POINT{ .x = starting.x, .y = button_size.cy });
      ...
      controller_controls.dpad_up_button.SetWindowPos(POINT{ .x = starting.x + button_size.cx });
      ...
      controller_controls.dpad_down_button.SetWindowPos(POINT{ .x = starting.x + button_size.cx, .y = button_size.cy + button_size.cy });
      ...
      controller_controls.dpad_right_button.SetWindowPos(POINT{ .x = starting.x + button_size.cx + button_size.cx, .y = button_size.cy });

      starting = POINT{ .x = starting.x + button_size.cx * 4 };

      controller_controls.left_x_axis.SetWindowStyle(controller_controls.left_x_axis.GetWindowStyle() | WS_VISIBLE);
      controller_controls.left_y_axis.SetWindowStyle(controller_controls.left_y_axis.GetWindowStyle() | WS_VISIBLE);

      RECT rect{};
      controller_controls.left_x_axis.SetWindowPos(slider_x_size);
      ::SendMessageW(controller_controls.left_x_axis, TBM_GETTHUMBRECT, 0, (LPARAM)&rect);
      slider_x_size = SIZE{ .cx = slider_x_size.cx, .cy = rect.bottom - rect.top };
      controller_controls.left_x_axis.SetWindowPos(slider_x_size);

      controller_controls.left_y_axis.SetWindowPos(slider_y_size);
      ::SendMessageW(controller_controls.left_y_axis, TBM_GETTHUMBRECT, 0, (LPARAM)&rect);
      slider_y_size = SIZE{ .cx = rect.right - rect.left, .cy = slider_y_size.cy };
      controller_controls.left_y_axis.SetWindowPos(slider_y_size);

      controller_controls.left_x_axis.SetWindowPos(POINT{ .x = starting.x, .y = (slider_y_size.cy / 2) - (slider_x_size.cy / 2) });
      controller_controls.left_y_axis.SetWindowPos(POINT{ .x = starting.x + (slider_x_size.cx / 2) - (slider_y_size.cx / 2) });

      starting = POINT{ .x = starting.x + button_size.cx * 4 };
      slider_x_size = SIZE{ .cx = button_size.cx * 3, .cy = button_size.cy };
      slider_y_size = SIZE{ .cx = button_size.cx, .cy = button_size.cy * 3 };

      controller_controls.right_x_axis.SetWindowStyle(controller_controls.right_x_axis.GetWindowStyle() | WS_VISIBLE);
      controller_controls.right_y_axis.SetWindowStyle(controller_controls.right_y_axis.GetWindowStyle() | WS_VISIBLE);

      controller_controls.right_x_axis.SetWindowPos(slider_x_size);
      ::SendMessageW(controller_controls.right_x_axis, TBM_GETTHUMBRECT, 0, (LPARAM)&rect);
      slider_x_size = SIZE{ .cx = slider_x_size.cx, .cy = rect.bottom - rect.top };
      controller_controls.right_x_axis.SetWindowPos(slider_x_size);

      controller_controls.right_y_axis.SetWindowPos(slider_y_size);
      ::SendMessageW(controller_controls.right_y_axis, TBM_GETTHUMBRECT, 0, (LPARAM)&rect);
      slider_y_size = SIZE{ .cx = rect.right - rect.left, .cy = slider_y_size.cy };
      controller_controls.right_y_axis.SetWindowPos(slider_y_size);

      controller_controls.right_x_axis.SetWindowPos(POINT{ .x = starting.x, .y = (slider_y_size.cy / 2) - (slider_x_size.cy / 2) });
      controller_controls.right_y_axis.SetWindowPos(POINT{ .x = starting.x + (slider_x_size.cx / 2) - (slider_y_size.cx / 2) });
      ...
      controller_controls.x_button.SetWindowPos(POINT{ .x = starting.x, .y = button_size.cy });
      ...
      controller_controls.y_button.SetWindowPos(POINT{ .x = starting.x + button_size.cx });
      ...
      controller_controls.a_button.SetWindowPos(POINT{ .x = starting.x + button_size.cx, .y = button_size.cy + button_size.cy });
      ...
      controller_controls.b_button.SetWindowPos(POINT{ .x = starting.x + button_size.cx + button_size.cx, .y = button_size.cy });

      return 0;
    }

    std::optional<win32::lresult_t> wm_input()
    {
