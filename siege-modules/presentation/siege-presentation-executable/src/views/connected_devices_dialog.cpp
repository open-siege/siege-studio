...
namespace siege::views
{
  ...
  controller_info store_controller_context(const controller_info& info, hardware_context context);
  ...
  struct connected_devices_dialog : win32::basic_window<connected_devices_dialog>
  {
    ...
    std::vector<ui_controller_mapping> mappings;
    std::span<ui_controller_mapping> button_mappings;
    std::span<ui_controller_mapping> axis_mappings;
    ...
    LRESULT wm_create()
    {
      ...
      device_context.bind_nm_click([this](auto device_context, const auto& message) {
        ...
        controller->second.state.info = store_controller_context(controller->second.state.info, (siege::platform::hardware_context)message.dwItemSpec);
      ...
      custom_mapping.SetExtendedListViewStyle(0, LVS_EX_FULLROWSELECT | LVS_EX_DOUBLEBUFFER);
      ...
      custom_mapping.bind_nm_dbl_click([this](win32::list_view custom_mapping, const NMITEMACTIVATE& message) {
        ...
        if (controller->second.state.info.detected_context != siege::platform::hardware_context::custom)
        {
          return;
        }

        auto deferred = std::shared_ptr<void>{
          nullptr, [this, &controller](...) {
            register_for_raw_input();

            std::vector<input_mapping> real_mappings;
            real_mappings.reserve(std::ranges::count_if(mappings, [](auto& mapping) {
            ...
            for (auto& mapping : mappings)
            {
              ...
              auto& back = real_mappings.emplace_back();
              back.vkey = mapping.vkey;
              back.index = mapping.index;
            }

            auto func = store_custom_controller_mapping(controller->second.state.info, real_mappings);

            if (func)
            {
              controller->second.state.info.get_hardware_index = func;
              refresh_ui(controller->second.state);
            }
          }
        };
        ...
        auto button_or_axis = stl::find_if(mappings, [&](auto& mapping) {
          return mapping.item == message.iItem;
        });
        if (button_or_axis != mappings.end() && (button_or_axis->vkey == VK_GAMEPAD_LEFT_TRIGGER || button_or_axis->vkey == VK_GAMEPAD_RIGHT_TRIGGER))
        {
          auto selected_item = get_raw_input_from_dialog(this->ref(), controller->first, controller->second.state, std::nullopt);
          if (!selected_item)
          {
            return;
          }

          auto item_to_swap = stl::find_if(mappings, [&](auto& mapping) {
            return mapping.index.index == selected_item->hardware_index && mapping.index.type == selected_item->input_type;
          });

          if (item_to_swap != mappings.end() && item_to_swap->index.type == button_or_axis->index.type)
          {
            item_to_swap->index.index = button_or_axis->index.index;
          }

          button_or_axis->index.index = selected_item->hardware_index;
          button_or_axis->index.type = selected_item->input_type;
          return;
        }

        auto button = stl::find_if(button_mappings, [&](auto& mapping) {
          return mapping.item == message.iItem;
        });

        if (button != button_mappings.end())
        {
          auto selected_button = get_raw_input_from_dialog(this->ref(), controller->first, controller->second.state, input_type::button);

          if (!selected_button)
          {
            return;
          }
          auto button_to_swap = stl::find_if(button_mappings, [&](auto& mapping) {
            return mapping.index.index == selected_button->hardware_index && mapping.index.type == selected_button->input_type;
          });

          if (button_to_swap != button_mappings.end())
          {
            button_to_swap->index.index = button->index.index;
            std::swap(button_to_swap->item, button->item);
          }

          button->index.index = selected_button->hardware_index;

          return;
        }

        auto axis = stl::find_if(axis_mappings, [&](auto& mapping) {
          return mapping.item == message.iItem;
        });

        if (axis == axis_mappings.end())
        {
          return;
        }


        auto selected_axis = get_raw_input_from_dialog(this->ref(), controller->first, controller->second.state, input_type::axis);
        ...
        auto axis_to_swap = stl::find_if(axis_mappings, [&](auto& mapping) {
          return mapping.index.index == selected_axis->hardware_index && mapping.index.type == selected_axis->input_type;
        });

        if (axis_to_swap != axis_mappings.end())
        {
          axis_to_swap->index.index = axis->index.index;
          std::swap(axis_to_swap->item, axis->item);
        }

        axis->index.index = selected_axis->hardware_index;
      });

      for (auto i = 1; i <= 32; ++i)
      {
        win32::list_view_item button_item(L"N/A");
        button_item.sub_items.emplace_back(L"Button " + std::to_wstring(i));
      ...
      for (auto i = 1; i <= 16; ++i)
      {
        win32::list_view_item axis_item(L"N/A");
        axis_item.sub_items.emplace_back(L"Axis " + std::to_wstring(i));
      ...
      button_mappings = std::span<ui_controller_mapping>(mappings).subspan(0, 32);
      axis_mappings = std::span<ui_controller_mapping>(mappings).subspan(32);
    ...
    void refresh_ui(controller_state& state)
    {
      for (auto& mapping : mappings)
      {
      ...
      for (auto button : hardware_buttons(state))
      {
        auto mapping = stl::find_if(button_mappings, [button](auto& mapping) {
          return mapping.index.index == button;
        });

        if (mapping == button_mappings.end())
        {
          continue;
        }
      ...
      for (auto axis : hardware_axes(state))
      {
        auto mapping = stl::find_if(axis_mappings, [axis](auto& mapping) {
          return mapping.index.index == axis;
        });

        if (mapping == axis_mappings.end())
        {
          continue;
        }
        mapping->visible = true;
      ...
      {
        if (!state.info.get_hardware_index)
        {
          break;
        }

        auto index = state.info.get_hardware_index(button, controller_info::prefer_button);

        if (!index.has_value() || index->type != input_type::button)
        {
          continue;
        }
           ...
           })
      {
        ...
        auto index = state.info.get_hardware_index(axis, controller_info::prefer_axis);

        if (!index.has_value() || index->type != input_type::axis)
        {
          continue;
        }
