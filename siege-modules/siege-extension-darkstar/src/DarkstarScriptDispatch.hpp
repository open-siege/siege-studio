...
{
	struct DarkstarScriptDispatch final : ScriptDispatchBase
	{
		...
		HRESULT __stdcall Invoke(DISPID dispIdMember, const GUID& riid, LCID lcid, WORD wFlags, DISPPARAMS* pDispParams, VARIANT* pVarResult, EXCEPINFO* pExcepInfo, UINT* puArgErr) override
		{
			...
			for (auto i = 0u; i < pDispParams->cArgs; ++i)
			{
				...
				else if (pDispParams->rgvarg[i].vt == VT_BOOL)
				{
					auto value = win32::com::to_string(pDispParams->rgvarg[i]);
					value[0] = (char)std::toupper(value[0]);
					exec_string.append(std::move(value));
				}
				else
				{
