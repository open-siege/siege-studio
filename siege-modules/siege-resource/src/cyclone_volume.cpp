...
namespace siege::resource::cln
{
  ...
  std::vector<cln_file_archive::content_info> cln_file_archive::get_content_listing(std::istream& stream, const platform::listing_query& query) const
  {
    ...
    if (folder_entry != entries.end())
    {
      for (const auto& entry : entries)
      {
        ...
        if (full_path.parent_path() == query.folder_path)
        {
          if (entry.offset == 0 && entry.file_size == 0)
          {
            siege::platform::folder_info temp{};
          ...
          else if (entry.flags == 0)
          {
            ...
            temp.compression_type = platform::compression_type::none;
  ...
  void cln_file_archive::set_stream_position(std::istream& stream, const siege::platform::file_info& info) const
  {
