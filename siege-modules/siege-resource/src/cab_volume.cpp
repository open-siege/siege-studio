...
namespace siege::resource::cab
{
  ...
  std::vector<cab_file_archive::content_info> cab_file_archive::get_content_listing(std::istream& stream, const platform::listing_query& query) const
  {
    return cab_get_content_listing(query);
  }

  void cab_file_archive::set_stream_position(std::istream& stream, const siege::platform::file_info& info) const
  {
