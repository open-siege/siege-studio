#include <siege/content/fnt/font.hpp>

namespace siege::content::fnt
{
  namespace endian = siege::platform;
  constexpr auto pft_tag = platform::to_tag<4>({ 'P', 'F', 'O', 'N' });
  ...
  bmp::pbmp_data read_phoenix_font(std::istream& raw_data)
  {
    ...
    raw_data.read(reinterpret_cast<char*>(file_header.data()), sizeof(file_header));
    raw_data.read(reinterpret_cast<char*>(&file_size), sizeof(file_size));
    raw_data.read(reinterpret_cast<char*>(&version), sizeof(version));
    ...
    raw_data.read(reinterpret_cast<char*>(&size), sizeof(size));

    endian::little_int32_t offset;
    raw_data.read(reinterpret_cast<char*>(&offset), sizeof(offset));

    raw_data.seekg(offset * sizeof(endian::little_int16_t));

    std::vector<endian::little_int16_t> chars(size);
    raw_data.read(reinterpret_cast<char*>(chars.data()), chars.size() * sizeof(endian::little_int16_t));

    std::vector<phoenix_char_info> info(header.char_count);
    raw_data.read(reinterpret_cast<char*>(info.data()), info.size() * sizeof(phoenix_char_info));
