...
{
  const static std::map<siege::platform::compression_type, const char*> type_names{
    { siege::platform::compression_type::none, "None" },
    { siege::platform::compression_type::lz, "Lempel-Ziv" },
    { siege::platform::compression_type::lzh, "Lempel-Ziv w/ Huffman coding" },
    { siege::platform::compression_type::rle, "Run-Length Encoding" }
  };
    ...
    )
  {
    ...
    if (search_text.has_value())
    {
      for (auto& file : files)
      {
        ...
        auto folder_name = platform::to_lower(std::filesystem::relative(file.folder_path, archive_path).string());
        auto lower_search = platform::to_lower(search_text.value());
  ...
  void vol_view::setup_view(wxWindow& parent)
  {
    ...
    export_all_button->Bind(wxEVT_BUTTON, [parent = &parent, this, folder_picker](wxCommandEvent& event) {
      ...
      pending_save = std::async(std::launch::async, [this, dialog = dialog.release(), folder_picker, text1 = text1.release(), text2 = text2.release(), gauge = gauge.release()] {
        ...
        std::vector<std::pair<std::filesystem::path, std::vector<siege::platform::file_info>>> found_files(all_files.size());
