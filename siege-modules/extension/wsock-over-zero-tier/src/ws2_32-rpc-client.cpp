...
int __stdcall siege_WSAStartup(WORD version, LPWSADATA data)
{
  ...
  if (env_size <= 1)
  {
    // System / no-backend path: drop any stale RPC server handle from a prior run.
    server_info.server = nullptr;
    return result;
  }
  ...
  // Tests (and restarts) may kill the server process while this DLL stays loaded.
  if (server_info.server && !::IsWindow(server_info.server))
  {
    server_info.server = nullptr;
  }
