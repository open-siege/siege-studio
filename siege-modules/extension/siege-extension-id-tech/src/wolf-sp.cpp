...
extern auto game_actions = std::array<game_action, 32>{ {
  ...
  game_action{ game_action::digital, "+sprint", u"Sprint", u"Movement" },
  ...
  game_action{ game_action::digital, "+kick", u"Kick", u"Combat" },
  game_action{ game_action::digital, "+zoom", u"Binocular Zoom", u"Combat" },
  game_action{ game_action::digital, "zoomin", u"Zoom In", u"Combat" },
  game_action{ game_action::digital, "zoomout", u"Zoom Out", u"Combat" },
...
std::errc init_mouse_inputs(mouse_binding* binding)
{
  ...
  if (auto config = load_config_from_pk3(L"Main\\default.cfg", L"Main/pak0.pk3", L"Main/pak0.pk3"))
  {
    upsert_mouse_bindings(*config, *binding);
  }

  if (auto config = load_config_from_file(L"Main\\wolfconfig.cfg"))
  {
    upsert_mouse_bindings(*config, *binding);
  }
  ...
  std::array<std::pair<WORD, std::string_view>, 23> actions{
    {
      ...
      std::make_pair<WORD, std::string_view>(VK_GAMEPAD_MENU, "notebook"),
    }
  };
