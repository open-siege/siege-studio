...
std::errc init_mouse_inputs(mouse_binding* binding)
{
  ...
  if (auto config = load_config_from_pk3(L"BaseEF\\default.cfg", L"BaseEF/pak0.pk3", L"BaseEF/pak0.pk3"))
  {
    upsert_mouse_bindings(*config, *binding);
  }

  if (auto config = load_config_from_file(L"BaseEF\\efconfig.cfg"))
  {
    upsert_mouse_bindings(*config, *binding);
  }
