...
constexpr std::array<std::array<std::pair<std::string_view, std::size_t>, 1>, 1> verification_strings = { { std::array<std::pair<std::string_view, std::size_t>, 1>{ { { "Medal of Honor Allied Assault(TM) Spearhead"sv, std::size_t(0x57cf90) } } } } };
...
std::errc apply_prelaunch_settings(const wchar_t* exe_path_str, siege::platform::game_command_line_args* args)
{
  ...
  std::ofstream custom_bindings("mainta/siege_studio_inputs.cfg", std::ios::binary | std::ios::trunc);
...
std::errc init_mouse_inputs(mouse_binding* binding)
{
  ...
  if (auto config = load_config_from_pk3(L"mainta\\default.cfg", L"mainta/pak0.pk3", L"mainta/pak0.pk3"))
  {
    upsert_mouse_bindings(*config, *binding);
  }

  if (auto config = load_config_from_file(L"mainta\\configs\\unnamedsoldier.cfg"))
  {
    upsert_mouse_bindings(*config, *binding);
  }
  ...
  get_predefined_int_command_line_settings(const wchar_t* name) noexcept
{
  ...
  {
    static auto modes = std::array<predefined_int, 11>{
      ...
      predefined_int{ .label = L"2048x1536", .value = 10 },
      predefined_int{ .label = L"856x480", .value = 11 },
  ...
  get_predefined_string_command_line_settings(const wchar_t* name) noexcept
{
  if (name && std::wstring_view(name) == L"map")
  {
    ...
    return get_predefined_id_tech_3_map_command_line_settings_multiple({ L"main", L"mainta" }, [](std::wstring_view label) {
