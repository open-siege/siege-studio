...
const wchar_t** format_command_line_sof(const siege::platform::game_command_line_args* args, std::uint32_t* new_size)
{
  if (!args)
  {
    return nullptr;
  ...
  if (connect_iter != args->string_settings.end())
  {
    bool is_local = connect_iter->value == L"0.0.0.0"sv || connect_iter->value == L"";
    bool is_deathmatch = deathmatch_iter != args->int_settings.end();

    if (!(is_local && is_deathmatch))
    {
      string_args.emplace_back(L"+connect");
      string_args.emplace_back(connect_iter->value);
    }
