...
constexpr static std::array<std::pair<std::string_view, std::string_view>, 8> variable_name_ranges{ {
  ...
  { "bot_testhidepos"sv, "bot_debug"sv },
...
std::errc init_mouse_inputs(mouse_binding* binding)
{
  ...
  if (auto config = load_config_from_pak(L"Main\\default.cfg", L"Main/pak0.pk3", L"Main/pak0.pk3"))
  {
    upsert_mouse_bindings(*config, *binding);
  }

  if (auto config = load_config_from_file(L"Main\\wolfconfig_mp.cfg"))
  {
    upsert_mouse_bindings(*config, *binding);
  }
