...
extern auto command_line_caps = game_command_line_caps{
  .int_settings = { { L"dedicated", L"r_customwidth", L"r_customheight", L"r_mode", L"in_joystick" } },
...
constexpr static std::array<std::pair<std::string_view, std::string_view>, 9> function_name_ranges{ {
  { "-mlook"sv, "+button0"sv },
  { "-force_drain"sv, "+useforce"sv },
  { "-altattack"sv, "+force_grip"sv },
  { "-use"sv, "centerview"sv },
  { "use_cloak"sv, "use_sentry"sv },
  { "zoom"sv, "use_seeker"sv },
  { "force_seeing"sv, "force_heal"sv },
  { "engage_duel"sv, "sv_saberswitch"sv },
  { "r_atihack"sv, "r_atihack"sv },
} };

constexpr static std::array<std::pair<std::string_view, std::string_view>, 4> variable_name_ranges{ {
  { "com_buildScript"sv, "com_speeds"sv },
...
std::errc apply_prelaunch_settings(const wchar_t* exe_path_str, siege::platform::game_command_line_args* args)
{
  ...
  if (enable_controller)
  {
    ...
    config.emplace(siege::configuration::key_type({ "seta", "joy_ybutton" }), siege::configuration::key_type("1"));
  }
...
std::errc init_mouse_inputs(mouse_binding* binding)
{
  ...
  if (auto config = load_config_from_pk3(L"base\\mpdefault.cfg", L"base/assets2.pk3", L"base/assets2.pk3"))
  {
    upsert_mouse_bindings(*config, *binding);
  }

  if (auto config = load_config_from_file(L"base\\jampconfig.cfg"))
  {
    upsert_mouse_bindings(*config, *binding);
  }
...
std::errc init_keyboard_inputs(keyboard_binding* binding)
{
  ...
  if (auto config = load_config_from_pk3(L"base\\mpdefault.cfg", L"base/assets0.pk3", L"base/assets0.pk3"))
  {
