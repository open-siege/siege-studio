...
extern auto command_line_caps = game_command_line_caps{
  .int_settings = { { L"dedicated", L"gl_mode", L"in_joystick" } },
...
constexpr static std::array<std::pair<std::string_view, std::string_view>, 4> function_name_ranges{ { { "-klook"sv, "centerview"sv },
  { "+mlook"sv, "-mlook"sv },
  { "wait"sv, "cmdlist"sv },
  { "echo"sv, "print"sv } } };
...
std::errc apply_prelaunch_settings(const wchar_t* exe_path_str, siege::platform::game_command_line_args* args)
{
  ...
  for (auto& alias : kingpin_aliases)
  {
...
std::errc init_mouse_inputs(mouse_binding* binding)
{
  ...
  if (auto config = load_config_from_pak(L"main\\default.cfg", L"main/Pak0.pak", L"base/Pak0.pak"))
  {
...
std::errc default_controller_inputs(controller_binding* binding, int layout_index)
{
  if (binding == nullptr)
  {
    return std::errc::bad_address;
  ...
  get_predefined_string_command_line_settings(const wchar_t* name) noexcept
{
  ...
  {
    return get_predefined_id_tech_2_map_command_line_settings(L"main", true);
  }
