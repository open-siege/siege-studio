...
constexpr std::array<std::array<std::pair<std::string_view, std::size_t>, 3>, 1> verification_strings = { { std::array<std::pair<std::string_view, std::size_t>, 3>{ { { "exec"sv, std::size_t(0x50369c) },
  { "cmdlist"sv, std::size_t(0x515ec8) },
  { "cl_pitchspeed"sv, std::size_t(0x5172a0) } } } } };
...
std::errc apply_prelaunch_settings(const wchar_t* exe_path_str, siege::platform::game_command_line_args* args)
{
  ...
  unbind_joystick_for_quake_3_config(config, raven_mapping_context{});
...
std::errc init_mouse_inputs(mouse_binding* binding)
{
  ...
  if (auto config = load_config_from_pk3(L"base\\default.cfg", L"base/assets2.pk3", L"base/assets2.pk3"))
  {
    upsert_mouse_bindings(*config, *binding);
  }

  if (auto config = load_config_from_file(L"base\\jaconfig.cfg"))
  {
    upsert_mouse_bindings(*config, *binding);
  }
  ...
  get_predefined_int_command_line_settings(const wchar_t* name) noexcept
{
  ...
  if (name_str == L"r_mode")
  {
    static auto modes = std::array<predefined_int, 9>{
      ...
      predefined_int{ .label = L"800x600", .value = 2 },
      predefined_int{ .label = L"960x720", .value = 3 },
      predefined_int{ .label = L"1024x768", .value = 4 },
      predefined_int{ .label = L"1152x864", .value = 5 },
      predefined_int{ .label = L"1280x1024", .value = 6 },
      predefined_int{ .label = L"1600x1200", .value = 7 },
