...
std::errc executable_is_supported(const wchar_t* filename) noexcept
{
  ...
  if (exe_path.stem() == "CoDUOMP" && exe_path.extension() == ".exe" && std::filesystem::exists(parent_path / "uo_gamex86.dll", last_error) && std::filesystem::exists(parent_path / "uo_cgamex86.dll", last_error) && std::filesystem::exists(parent_path / "uo_uix86.dll", last_error) && std::filesystem::is_directory(parent_path / "uo", last_error))
  {
    return std::errc{};
  }
...
std::errc init_mouse_inputs(mouse_binding* binding)
{
  ...
  if (auto config = load_config_from_pk3(L"uo\\default_mp.cfg", L"uo/localized_english_pakuo00.pk3", L"uo/localized_english_pakuo00.pk3"))
  {
    upsert_mouse_bindings(*config, *binding);
  }

  if (auto config = load_config_from_file(L"uo\\uoconfig_mp.cfg"))
  {
    upsert_mouse_bindings(*config, *binding);
  }
