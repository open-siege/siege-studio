...
namespace siege::views
{
  ...
  void pal_mapping_view::setup_view(wxWindow& parent)
  {
    ...
    search->Bind(wxEVT_SEARCH, [sizer = sizer.get(),
      ...
      auto search_value = platform::to_lower(event.GetString().utf8_string());
      ...
      for (auto& image : images)
      {
        auto filename = platform::to_lower(image.filename.string());
        auto folder_name = platform::to_lower(std::filesystem::relative(image.folder_path, context.explorer.get_search_path()).string());
