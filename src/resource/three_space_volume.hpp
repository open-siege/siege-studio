...
{
  struct rmf_file_archive : studio::resource::archive_plugin
  {
    ...
    std::vector<std::variant<studio::resource::folder_info, studio::resource::file_info>> get_content_listing(std::basic_istream<std::byte>& stream, std::filesystem::path archive_or_folder_path) const override;
  ...
  struct dyn_file_archive : studio::resource::archive_plugin
  {
    static bool is_supported(std::basic_istream<std::byte>& stream);
  ...
  struct vol_file_archive : studio::resource::archive_plugin
  {
    static bool is_supported(std::basic_istream<std::byte>& stream);
