...
namespace studio::configurations::id_tech
{
    ...
    namespace id_tech_1
    {
        ...
        std::optional<game_config> load_config(std::istream& raw_data, std::size_t stream_size)
        {
            ...
            bool is_valid = result.raw_data.find("snd_channels\t\t") != std::string::npos &&
                            result.raw_data.find("snd_musicdevice\t\t") != std::string::npos &&
                            result.raw_data.find("use_mouse\t\t") != std::string::npos;
