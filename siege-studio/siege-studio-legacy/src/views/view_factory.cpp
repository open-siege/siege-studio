...
namespace siege::views
{
  ...
  studio_view view_factory::create_view(const siege::platform::file_info& file_info, std::istream& stream, const siege::resource::resource_explorer& manager) const
  {
    auto search_values = std::array<std::string, 2>{{platform::to_lower(file_info.filename.extension().string()), platform::to_lower(file_info.filename.string())}};
  ...
  studio_view  view_factory::create_default_view(const siege::platform::file_info& file_info, std::istream& stream, const siege::resource::resource_explorer& manager) const
  {
    return normal_view(default_view(file_info));
  }
}// namespace siege::views
