...
namespace siege
{
  siege::resource::file_stream create_null_stream()
  {
    static siege::resource::null_buffer null_buffer;
    return std::make_pair(siege::platform::file_info{}, std::make_unique<std::istream>(&null_buffer));
  }
  ...
  struct tree_item_file_info : public wxTreeItemData
  {
    ...
    explicit tree_item_file_info(siege::platform::file_info info) : info(std::move(info)) {}
  };
  ...
  struct tree_item_folder_info : public wxTreeItemData
  {
    ...
    explicit tree_item_folder_info(siege::platform::folder_info info) : info(std::move(info)) {}
  };
  ...
  void create_render_view(wxWindow& panel, siege::resource::file_stream file_stream, const views::view_factory& factory, const siege::resource::resource_explorer& archive)
  {
...
// This can be addressed as part of the planned reworking.
      ...
      files_folders.begin(), files_folders.end(), [](const auto& a, const auto& b) {
        return std::visit([&](const auto& entry) {
          ...
          return std::visit([&](const auto& other) {
            ...
            if constexpr (std::is_same_v<AType, siege::platform::folder_info> && std::is_same_v<BType, AType>)
            {
              return entry.full_path < other.full_path;
            }

            if constexpr (std::is_same_v<AType, siege::platform::file_info> && std::is_same_v<BType, AType>)
            {
              return entry.filename < other.filename;
            }

            if constexpr (std::is_same_v<AType, siege::platform::file_info> && std::is_same_v<BType, siege::platform::folder_info>)
            {
              return false;
            }

            if constexpr (std::is_same_v<AType, siege::platform::folder_info> && std::is_same_v<BType, siege::platform::file_info>)
            {
              return true;
            }
      ...
      std::visit([&](auto&& file) {
        ...
        if constexpr (std::is_same_v<T, siege::platform::file_info>)
        {
          if (std::any_of(extensions.begin(), extensions.end(), [&file](const auto& ext) {
                return shared::ends_with(platform::to_lower(file.filename.string()), ext);
              }))
          {
            if (file_filter.has_value() && !file_filter.value().empty())
            {
              auto filename = platform::to_lower(file.filename.string());
              auto lower_search = platform::to_lower(file_filter.value());
  ...
  auto create_navigation_panel(std::shared_ptr<wxFrame> frame,
    ...
    siege::resource::resource_explorer& archive,
    std::function<void(siege::resource::file_stream, bool)> add_element_from_file)
  {
    ...
    auto tree_search = std::shared_ptr<wxComboBox>(new wxComboBox(tree_panel.get(), wxID_ANY), siege::default_wx_deleter);
    ...
    auto tree_view = std::shared_ptr<wxTreeCtrl>(new wxTreeCtrl(tree_panel.get()), siege::default_wx_deleter);

    siege::populate_tree_view(view_factory, archive, *tree_view, fs::current_path(), extensions);
    ...
    auto search = std::shared_ptr<wxSearchCtrl>(new wxSearchCtrl(tree_panel.get(), wxID_ANY), siege::default_wx_deleter);
    search->ShowSearchButton(true);
    search->ShowCancelButton(true);

    auto repopulate = [&view_factory, &archive, tree_view, search, get_filter_selection]() {
      siege::populate_tree_view(view_factory, archive, *tree_view, fs::current_path(), get_filter_selection(), std::nullopt, search->GetValue().utf8_string());
    };
    ...
    tree_view->Bind(wxEVT_TREE_ITEM_EXPANDING, [&view_factory, &archive, tree_view, get_filter_selection](wxTreeEvent& event) {
      ...
      if (tree_view->HasChildren(item))
      {
        ...
        {
          if (auto* real_info = dynamic_cast<siege::tree_item_folder_info*>(tree_view->GetItemData(child)); real_info)
          {
            siege::populate_tree_view(view_factory, archive, *tree_view, real_info->info.full_path, get_filter_selection(), child);
          }
        }
    ...
    tree_view->Bind(wxEVT_TREE_ITEM_ACTIVATED, [&archive, tree_view, add_element_from_file](wxTreeEvent& event) {
      ...
      if (auto* real_info = dynamic_cast<siege::tree_item_file_info*>(tree_view->GetItemData(item)); real_info)
      {
        add_element_from_file(archive.load_file(real_info->info), had_first_activation == false);
        had_first_activation = true;
      }
      else if (auto* folder_info = dynamic_cast<siege::tree_item_folder_info*>(tree_view->GetItemData(item));
...
int main(int argc, char** argv)
{
  try
  {
    auto archive = siege::views::create_default_resource_explorer();
    auto view_factory = siege::views::create_default_view_factory(archive);

    view_factory.get_actions().get_extensions_by_category = [&](auto path) {
      return view_factory.get_extensions_by_category(path);
    };

    wxApp::SetInitializerFunction(siege::createApp);
    wxEntryStart(argc, argv);
    auto* app = wxApp::GetInstance();
    app->CallOnInit();
    wxInitAllImageHandlers();

    auto frame = std::shared_ptr<wxFrame>(new wxFrame(nullptr, wxID_ANY, "Siege Studio"), siege::default_wx_deleter);
    ...
    auto notebook = std::shared_ptr<wxAuiNotebook>(new wxAuiNotebook(frame.get(), wxID_ANY), siege::default_wx_deleter);
    ...
    auto add_element_from_file = [notebook, frame, &num_elements, &view_factory, &archive](auto new_stream, bool replace_selection = false) {
      ...
      siege::create_render_view(*panel, std::move(new_stream), view_factory, archive);
    ...
    auto add_new_element = [notebook, &num_elements, &view_factory, &archive]() {
      ...
      auto panel = std::make_unique<wxPanel>(notebook.get(), wxID_ANY);
      panel->SetSizer(std::make_unique<wxBoxSizer>(wxHORIZONTAL).release());
      siege::create_render_view(*panel, siege::create_null_stream(), view_factory, archive);
    ...
    menuTab->Append(siege::event_close_current_tab, "Close");
    menuTab->Append(siege::event_close_other_tabs, "Close Other Tabs");
    menuTab->Append(siege::event_close_left_tabs, "Close Tabs to the Left");
    menuTab->Append(siege::event_close_right_tabs, "Close Tabs to the Right");
    ...
    notebook->Bind(
      ...
      siege::event_close_other_tabs);

    notebook->Bind(wxEVT_MENU, close_left_tabs,siege::event_close_left_tabs);
    notebook->Bind(wxEVT_MENU, close_right_tabs,siege::event_close_right_tabs);
    ...
    auto navigation_panel_data = siege::create_navigation_panel(frame, view_factory, archive, add_element_from_file);
    auto tree_view = std::move(navigation_panel_data.first);
    auto repopulate = navigation_panel_data.second;

    frame->Bind(
      wxEVT_MENU, [&](auto& event) {
        const auto new_path = siege::get_shape_path(view_factory.get_extensions());
    ...
    frame->Bind(
      ...
      siege::event_open_in_new_tab);

    frame->Bind(
      wxEVT_MENU, [&](auto& event) {
        const auto new_path = siege::get_workspace_path();
      ...
      siege::event_open_folder_as_workspace);
    ...
    frame->SetMenuBar(siege::create_menu_bar().release());
