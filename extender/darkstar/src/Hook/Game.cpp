...
namespace Hook
{
  ...
  std::optional<Core::GameFunctions> loadFunctions(std::string functionsFileName = "config.json")
  {
    ...
    if (!functionData.contains("default"))
    {
      return std::nullopt;
    }

    if (functionData["default"].is_null())
    {
      return std::nullopt;
    }
  ...
  {
    auto functions = loadFunctions(std::move(configFilename));

    if (functions.has_value())
    {
      DarkstarSetGameFunctions(functions.value());
