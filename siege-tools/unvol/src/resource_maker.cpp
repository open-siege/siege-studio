...
namespace siege::resource
{
  ...
  {
    return vol::darkstar::is_stream_supported(stream) || 
        vol::three_space::is_stream_vol(stream) ||
        vol::three_space::is_stream_dyn(stream) || 
        vol::three_space::is_stream_rmf(stream) || 
        vol::trophy_bass::is_stream_rbx(stream) || 
        vol::trophy_bass::is_stream_tbv(stream) || 
        pak::is_stream_supported(stream) || 
        clm::is_stream_supported(stream) || 
        cln::is_stream_supported(stream) || 
        atd::is_stream_supported(stream) || 
        wad::is_stream_supported(stream) || 
        rsc::is_stream_supported(stream) || 
        res::is_stream_supported(stream) || 
        mw4::is_stream_supported(stream) || 
        prj::is_stream_supported(stream) || 
        zip::is_stream_zip(stream);
  }
